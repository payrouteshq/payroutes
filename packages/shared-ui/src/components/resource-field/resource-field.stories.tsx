...
import { ResourceField, ResourceFieldItem, ResourceFieldKey } from "./index";
...
function itemLabel(item: Resource) {
  ...
  return (
    <>
      {prefix}.<span className="text-muted-foreground">&lt;key&gt;</span>
    </>
  );
}
...
function ResourceFieldDemo({
  ...
  return (
    <ResourceField
      ...
      searchFilter={(item, query) => item.title.toLowerCase().includes(query.toLowerCase())}
...
export const Open: Story = {
  ...
  render: () => (
    <div className="min-h-96">
      <ResourceFieldDemo defaultOpen defaultValue={resources.find((item) => item.title === "product.id") ?? null} />
    </div>
  ),
};

export const Selected: Story = {
  render: () => <ResourceFieldDemo defaultValue={resources.find((item) => item.title === "product.id") ?? null} />,
};

export const WithKey: Story = {
  render: () => (
    <ResourceFieldDemo defaultValue={resources.find((item) => item.title === "product.metadata.<key>") ?? null} />
  ),
};

export const Item: Story = {
  render: () => (
    <div className="border-ring bg-popover w-72 overflow-hidden rounded-lg border-2 py-1">
      <div className="text-muted-foreground px-3 py-1.5 text-[10px] font-medium tracking-wider uppercase">Payment</div>
      ...
      <div className="text-muted-foreground px-3 py-1.5 text-[10px] font-medium tracking-wider uppercase">Product</div>
      <div className="flex flex-col gap-0.5 px-1">
