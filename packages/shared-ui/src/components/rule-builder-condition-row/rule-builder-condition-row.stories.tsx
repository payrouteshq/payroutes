    ...
    <RuleBuilderConditionRow
      ...
      onResourceChange={(item) =>
        onChange({
          ...value,
          resourceId: item?.id ?? "",
          fieldKey: item?.kind === "object" ? value.fieldKey : "",
        })
      }
      resources={resources}
      ...
      onOperatorChange={(operator) => onChange({ ...value, operator: operator ?? "is" })}
      value={value.value}
...
function FlatGroupDemo({ disabled, defaultConditions }: { disabled?: boolean; defaultConditions?: Condition[] }) {
  ...
  return (
    <RuleBuilderConditionGroup
      ...
      onCombinatorChange={(value) => form.setValue("combinator", value ?? "and")}
      onAddCondition={() => append(figCondition({ fieldKey: "", value: "" }))}
      disabled={disabled}
    >
...
export const Empty: Story = {
  render: () => (
    <FlatGroupDemo defaultConditions={[figCondition({ resourceId: "", fieldKey: "", operator: "is", value: "" })]} />
  ),
};
      ...
      <>
        {fields.map((field, index) => (
          <Controller
            ...
            name={`conditions.${index}`}
            render={({ field: rowField }) => (
              <ConditionRow value={rowField.value} onChange={rowField.onChange} onRemove={() => remove(index)} />
            )}
          />
        ))}
      </>
