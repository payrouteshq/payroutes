...
function ResourceFieldItem({
  ...
  return (
    <div
      ...
      className={cn(
        "group/row text-foreground relative flex w-full items-center justify-between overflow-hidden rounded-md py-2.5 pr-3 pl-3.5 text-left text-sm",
...
function ResourceFieldKey({
  ...
  return (
    <div data-slot="resource-field-key" className={cn("flex min-w-0 items-center gap-1", className)}>
      <PopoverTrigger
        type="button"
        className="border-input bg-popover text-foreground focus-visible:border-ring focus-visible:ring-ring/50 inline-flex h-9 min-w-0 items-center gap-1.5 rounded-lg border px-2.5 font-mono text-sm outline-none select-none focus-visible:ring-3"
      >
...
function ResourceField<T>({
  ...
  const isItemSelected = (item: T) => value != null && getItemTitle(value) === getItemTitle(item);
  ...
  const picker = (
    <PopoverContent
      ...
      className={cn("border-ring w-(--anchor-width) min-w-72 overflow-hidden border-2 p-0", popProps.className)}
    >
      <div className="p-2">
        <InputGroup className="border-input bg-card has-[:focus-visible]:border-ring rounded-lg shadow-none has-[:focus-visible]:ring-0">
        ...
        {grouped ? (
          grouped.map(([group, groupItems]) => (
            <div key={group} className="px-1">
              <div className="text-muted-foreground px-2 py-1.5 text-[10px] font-medium tracking-wider uppercase">
                {group}
              </div>
              <div className="flex flex-col gap-0.5">{groupItems.map(renderItemButton)}</div>
            </div>
          ))
        ) : (
          <>
            ...
            <div className="flex flex-col gap-0.5 px-1">{filtered.map(renderItemButton)}</div>
          </>
        )}

        {filtered.length === 0 ? (
          <div className="text-muted-foreground p-4 text-center text-xs">No results found</div>
        ) : null}
      </div>
    </PopoverContent>
  );
  ...
  const trigger = (text: string) => (
    <PopoverTrigger
      type="button"
      className={cn(
        "border-input bg-popover flex h-9 w-full items-center justify-between rounded-lg border px-2.5 text-sm transition-colors outline-none select-none",
    ...
    >
      ...
      {isLoading ? <Spinner size={20} /> : <ChevronsUpDown className="text-muted-foreground ml-2 size-4 shrink-0" />}
    </PopoverTrigger>
  );
        ...
        <div ref={setAnchor} className="flex min-w-0 items-center">
          {summary ?? (
            <PopoverTrigger type="button" className={selectedTriggerClassName}>
              ...
              {isLoading ? <Spinner size={20} /> : <ChevronDown className="text-muted-foreground size-3.5 shrink-0" />}
            </PopoverTrigger>
          )}
        </div>
      ) : (
  ...
  return (
    <div {...container} className={cn("flex w-full flex-col gap-1.5", container.className)}>
      {label ? (
      ...
      {field}
      {error ? (
        <p {...errorProps} className={cn("text-error text-sm", errorProps.className)}>
          {error}
        </p>
      ) : null}
    </div>
