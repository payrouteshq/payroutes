    ...
    <RuleBuilderConditionRow
      ...
      onResourceChange={(item) =>
        onChange({
          ...row,
          resourceId: item?.id ?? "",
          fieldKey: item?.kind === "object" ? row.fieldKey : "",
        })
      }
      resources={resources}
      ...
      onOperatorChange={(operator) => onChange({ ...row, operator: operator ?? "is" })}
      value={row.value}
...
function GroupDemo({ disabled }: { disabled?: boolean }) {
  ...
  return (
    <RuleBuilderConditionGroup
      ...
      onAddCondition={() => setRows((current) => [...current, figRow(String(Date.now()), { fieldKey: "", value: "" })])}
      onRemove={() => setRows([])}
      disabled={disabled}
    >
      {rows.map((row) => (
        <ConditionRow
          ...
          row={row}
          disabled={disabled}
          onChange={(next) => setRows((current) => current.map((item) => (item.id === next.id ? next : item)))}
...
function NestedDemo() {
  ...
  const [groups, setGroups] = React.useState<InnerGroup[]>([
    {
      id: "or-1",
      combinator: "or",
      rows: [figRow("2", { fieldKey: "region", value: "eu" }), figRow("3", { fieldKey: "region", value: "uk" })],
    },
  ]);
  ...
  return (
    <RuleBuilderConditionGroup
      ...
      onCombinatorChange={(value) => setCombinator(value ?? "and")}
      onAddCondition={() =>
        setOuterRows((current) => [...current, figRow(String(Date.now()), { fieldKey: "", value: "" })])
      }
      onAddGroup={() =>
        setGroups((current) => [
          ...current,
    ...
    >
      {outerRows.map((row) => (
        <ConditionRow
          key={row.id}
          row={row}
          onChange={(next) => setOuterRows((current) => current.map((item) => (item.id === next.id ? next : item)))}
          onRemove={() => setOuterRows((current) => current.filter((item) => item.id !== row.id))}
        />
      ))}
      {groups.map((group) => (
        <RuleBuilderConditionGroup
          ...
          onCombinatorChange={(value) =>
            setGroups((current) =>
              current.map((item) => (item.id === group.id ? { ...item, combinator: value ?? "or" } : item))
            )
          }
          onAddCondition={() =>
            setGroups((current) =>
              current.map((item) =>
                item.id === group.id
                  ? {
                      ...item,
                      rows: [...item.rows, figRow(String(Date.now()), { fieldKey: "", value: "" })],
                    }
                  : item
              )
            )
                ...
                setGroups((current) =>
                  current.map((item) =>
                    item.id === group.id
                      ? {
                          ...item,
                          rows: item.rows.map((entry) => (entry.id === next.id ? next : entry)),
                        }
                      : item
                  )
                )
